...
void Robot::print() const{
  ...
  cout<<" and I am facing ";
      if (orientation==east)cout<<"EAST\n";
      else if(orientation==south)cout<<"SOUTH\n";
      else if(orientation==west)cout<<"WEST\n";
      else if(orientation==north)cout<<"NORTH\n";
}
...
bool Robot::forward(){
  bool move =  false;
  if(orientation==east && rPos.getX()<9){
      rPos.set(rPos.getX()+1,rPos.getY());
      move = true;
  }
  else if(orientation==west&&rPos.getX()>0){
      rPos.set(rPos.getX()-1, rPos.getY());
      move = true;  
  }
  else if(orientation==north && rPos.getY()<9 ){
       rPos.set(rPos.getX(),rPos.getY()+1);
       move = true;
  }
  else if(orientation==south && rPos.getY()>0){
      rPos.set(rPos.getX(),rPos.getY()-1);
      move = true;
  }
return move;
